...
def cluster_rgb_points(points, rgb_image, n_clusters):
    rgb_values = rgb_image[points[:, 0], points[:, 1]]
    kmeans = KMeans(n_clusters=n_clusters, random_state=0)
    kmeans.fit(rgb_values)
    return kmeans.labels_


def cluster_sky_non_sky(features, image, n_clusters=5, sample_points=None, sample_labels=None):
    ...
    if sample_points is not None and sample_labels is not None:
        sky_points = np.array(sample_points)[np.array(sample_labels) == 1]
        tree_points = np.array(sample_points)[np.array(sample_labels) == 2]

        # Cluster sky points into 5 clusters based on RGB features
        sky_labels = cluster_rgb_points(sky_points, image, 3)
        sky_cluster_labels = sky_labels + 1  # Labels 1 to 3

        # Cluster tree points into 3 clusters based on RGB features
        # tree_labels = cluster_rgb_points(tree_points, image, 1)
        # tree_cluster_labels = tree_labels + 2  # Labels 4

        # Create a combined label array for initialization
        combined_labels = np.zeros(np.array(sample_points).shape[0], dtype=int)
        combined_labels[sample_labels == 1] = sky_cluster_labels
        combined_labels[sample_labels == 2] = 4
        combined_labels[sample_labels == 0] = 0  # Non-sky, non-tree

        sample_indices = np.array([point[0] * width + point[1] for point in sample_points])
        init_centers = np.array([features_flattened[sample_indices[combined_labels == label]].mean(axis=0)
    ...
    # Process the labels_image to map the clusters to sky, tree, and others
    final_labels_image = np.copy(labels_image)
    final_labels_image[np.isin(labels_image, range(1, 4))] = 1  # Sky
    final_labels_image[np.isin(labels_image, range(4, 5))] = 2  # Trees
    final_labels_image[labels_image == 0] = 0  # Non-sky, non-tree
...
def save_image_with_suffix(image_path, image):
    ...
    new_image_path = f"{base}_after{ext}"
...
image_path = "/home/kasra/PycharmProjects/DataFiltering1/d2d_post_processor/assets/12.jpg"
# Load the image
...
# Cluster the pixels
labels_image = cluster_sky_non_sky(feature_image, feature_image, sample_points=init_points,
